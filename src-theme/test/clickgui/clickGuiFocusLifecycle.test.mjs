...
test("virtual screen close releases focus before route listener cleanup", () => {
    ...
    const handlerStart = app.indexOf("async function handleVirtualScreen");
    const handlerEnd = app.indexOf("function registerThemeListener", handlerStart);
    const closeHandler = app.slice(handlerStart, handlerEnd);

    assert.ok(handlerStart >= 0, "App.svelte must handle virtual-screen close events");

    const releaseFocusIndex = closeHandler.indexOf("releaseDocumentFocus()");
    const routeChangeIndex = closeHandler.indexOf('await changeRoute("none")');
