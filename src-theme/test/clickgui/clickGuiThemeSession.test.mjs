...
test("session methods remain callable through the Modern preview adapter", async () => {
    const session = createClickGuiThemeSession({
        loadSettings: async () => classicSettings(),
        saveSettings: async () => {},
    });
    const {load, synchronize, setView, selectTheme, retryThemeSave} = session;

    assert.equal(await load(), true);
    setView("settings");
    synchronize(classicSettings());
    assert.equal(await selectTheme("Modern"), true);
    assert.equal(await retryThemeSave(), false);
    assert.equal(currentState(session).theme, "Modern");
});
