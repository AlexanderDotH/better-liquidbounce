...
class AlteningAccount(var accountToken: String) : MinecraftAccount(AccountService.THEALTENING) {
    ...
    override fun refresh() {
        val session = YggdrasilUserAuthentication(ALTENING_AUTH, ALTENING_AUTH_CLIENT)
            .authenticate(accountToken, "LiquidBounce")
    ...
    override fun fromRawJson(json: JsonObject) = json.run {
        readProfile()
        accessToken = string("token") ?: throw IllegalArgumentException("TheAltening account has no access token")
    ...
    companion object {
        ...
        private const val AUTH_TIMEOUT_SECONDS = 8L
        private val ALTENING_AUTH_CLIENT by lazy {
            HttpClient.client.newBuilder()
                ...
                .writeTimeout(AUTH_TIMEOUT_SECONDS, TimeUnit.SECONDS)
                .callTimeout(AUTH_TIMEOUT_SECONDS, TimeUnit.SECONDS)
                .build()
        }

        internal fun pending(accountToken: String, username: String) = AlteningAccount(accountToken).apply {
