...
sealed class MinecraftAccount(val service: AccountService) {
    ...
    protected open fun shouldRefreshBeforeLogin() = profile?.id == null
    ...
    protected fun JsonObject.readProfile() {
        username = string("name") ?: throw IllegalArgumentException("Account data has no account name")
    ...
    companion object {
        ...
        fun fromJson(json: JsonObject): MinecraftAccount {
            val serialName = json.string("type")
                ?: throw IllegalArgumentException("Account data has no account type")
