...
sealed class IsSelfBedMode(name: String, final override val parent: ModeValueGroup<*>) : Mode(name) {
    ...
    class Manual(parent: ModeValueGroup<*>) : IsSelfBedMode("Manual", parent) {
            ...
            when (event.key) {
                trackKey -> {
                    ...
                    val (bedPos, _) = center.searchBlocksInCuboid(16.0F) { _, state -> state.isBed }
                        ...
                        publishNotification(
                            "Cannot find any bed around you! Please get close to your bed.",
                ...
                untrackKey if trackedPos != BlockPos.ZERO -> {
                    ...
                    publishNotification(
                        "Bed position ($x, $y, $z) has been untracked.",
        ...
        private val worldHandler = handler<WorldChangeEvent> {
            val (x, y, z) = trackedPos
            publishNotification(
                "Bed position ($x, $y, $z) has been untracked due to world change.",
                NotificationEvent.Severity.INFO,
            )
            trackedPos.set(BlockPos.ZERO)
        }

        private fun publishNotification(message: String, severity: NotificationEvent.Severity) {
            EventManager.callEvent(NotificationEvent("SelfBed-$name", message, severity))
        }
