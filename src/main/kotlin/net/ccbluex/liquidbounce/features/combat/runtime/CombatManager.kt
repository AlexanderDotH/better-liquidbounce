...
import net.ccbluex.liquidbounce.features.combat.contract.CombatTargetSnapshots
...
object CombatManager : EventListener {
    ...
    init {
        CombatRuntimeEnvironment.bindRotationPaused { shouldPauseRotation }
    }
    ...
    val attackHandler = handler<AttackEntityEvent> { event ->
        ...
        if (entity is LivingEntity && entity.shouldBeAttacked()) {
            ...
            if (entity is Player) {
                EventManager.callEvent(TargetChangeEvent(CombatTargetSnapshots.fromPlayer(entity)))
            }
        }
    }
    ...
    val isInCombat: Boolean
        get() = this.duringCombat > 0 || CombatRuntimeEnvironment.hasActiveKillAuraTarget()
