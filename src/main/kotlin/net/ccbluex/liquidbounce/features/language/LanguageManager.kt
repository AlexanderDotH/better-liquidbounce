 ...
 */
package net.ccbluex.liquidbounce.features.language
...
import net.ccbluex.liquidbounce.lang.LanguageCatalogProvider
...
object LanguageManager : ValueGroup("Language") {
    ...
    init {
        LanguageCatalog.install(object : LanguageCatalogProvider {
            override fun translate(key: String, fallback: String): String =
                getLanguage()?.getOrDefault(key, fallback) ?: fallback

            override fun hasFallbackTranslation(key: String) = this@LanguageManager.hasFallbackTranslation(key)
        })
    }
    ...
    private val languageRegistry = ConcurrentHashMap<ClientLanguage, net.ccbluex.liquidbounce.features.language.ClientLanguage>()

    private fun loadLanguage(choice: ClientLanguage): net.ccbluex.liquidbounce.features.language.ClientLanguage? {
...
object LanguageLookupHook {
    @JvmStatic
    fun currentLanguage(): Language? = LanguageManager.getLanguage()
}
