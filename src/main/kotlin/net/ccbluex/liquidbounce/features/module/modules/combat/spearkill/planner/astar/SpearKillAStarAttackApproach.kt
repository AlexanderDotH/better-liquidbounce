...
internal fun isSpearKillAStarTerminalStepValid(
    ...
    for (step in outboundMovements.asReversed()) {
        if (!isSpearKillAStarTerminalMovementStepValid(step, stepLimit)) {
            return false
        }
...
private fun isSpearKillAStarTerminalMovementStepValid(step: Vec3, stepLimit: Double): Boolean =
    step.isFinite() && step.length() <= stepLimit + SPEAR_KILL_A_STAR_TERMINAL_SPEED_EPSILON
