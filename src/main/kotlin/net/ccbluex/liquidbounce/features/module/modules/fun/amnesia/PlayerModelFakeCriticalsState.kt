...
object PlayerModelFakeCriticalsState {
    ...
    private var mode = FakeCriticals.CriticalsMode.BOTH
    ...
    fun trigger(
        ...
        victim: LivingEntity,
        partialTicks: Float,
        mode: FakeCriticals.CriticalsMode,
        triggerWindow: Int,
        cooldown: Int,
    ...
    fun getTransform(
        ...
        val offset = if (velocityPositionActive) 0.0 else verticalOffset().toDouble()
    ...
    fun reset() {
        ...
        lastTriggerAt = 0L
        windowMs = 250
        mode = FakeCriticals.CriticalsMode.BOTH
    ...
    private fun verticalOffset(): Float {
        val progress = progress()
        val microHop = when (mode) {
            FakeCriticals.CriticalsMode.MICRO_HOP,
            FakeCriticals.CriticalsMode.BOTH -> sin(progress * PI).toFloat() * microHopHeight
            FakeCriticals.CriticalsMode.PACKET -> 0f
        }
        val packet = when (mode) {
            FakeCriticals.CriticalsMode.PACKET,
            FakeCriticals.CriticalsMode.BOTH -> packetOffset(progress)
            FakeCriticals.CriticalsMode.MICRO_HOP -> 0f
        }

        return microHop + packet
    }

    private fun packetOffset(progress: Float): Float {
        return when {
    ...
    private fun swingProgress(): Float = sin(progress() * PI).toFloat().coerceIn(0f, 1f)
