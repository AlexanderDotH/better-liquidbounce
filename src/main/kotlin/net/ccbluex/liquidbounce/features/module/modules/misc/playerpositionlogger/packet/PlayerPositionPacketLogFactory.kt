...
internal object PlayerPositionPacketLogFactory {
    fun outgoingMovement(
        ...
        val suppliedPosition = packet.takeIf { it.hasPosition() }?.let { LoggedVector(it.x, it.y, it.z) }
        val suppliedRotation = packet.takeIf { it.hasRotation() }?.let { LoggedRotation(it.yRot, it.xRot) }
        val resolvedPosition = LoggedVector(
            packet.getX(fallbackPosition.x), packet.getY(fallbackPosition.y), packet.getZ(fallbackPosition.z),
        )
        val resolvedRotation = LoggedRotation(
            packet.getYRot(fallbackRotation.yaw), packet.getXRot(fallbackRotation.pitch),
        )
    ...
    fun localCorrection(packet: ClientboundPlayerPositionPacket, source: PlayerPositionState) =
        PositionMoveRotation.calculateAbsolute(source.toPositionMoveRotation(), packet.change, packet.relatives).let {
            positionChange(
                PlayerPositionLogKind.LOCAL_CORRECTION,
                packet.change,
                it,
                packet.relatives.map { flag -> flag.name }.sorted(),
                teleportId = packet.id,
            )
        }

    fun remoteMovement(packet: ClientboundMoveEntityPacket, source: PlayerPositionState): PlayerPositionPacketObservation {
        val resolvedPosition = if (packet.hasPosition()) {
            VecDeltaCodec().apply { base = source.positionCodecBase.toVec3() }
                .decode(packet.xa.toLong(), packet.ya.toLong(), packet.za.toLong()).let(LoggedVector::from)
        } else source.position
        val resolvedRotation = if (packet.hasRotation()) {
            LoggedRotation(packet.yRot, packet.xRot)
        } else source.rotation.packetRotation
        return PlayerPositionPacketObservation(
    ...
    fun remoteTeleport(packet: ClientboundTeleportEntityPacket, source: PlayerPositionState) =
        PositionMoveRotation.calculateAbsolute(source.toPositionMoveRotation(), packet.change, packet.relatives).let {
            positionChange(
    ...
    fun localRotation(packet: ClientboundPlayerRotationPacket, source: PlayerPositionState): PlayerPositionPacketObservation {
    ...
    fun explosionKnockback(knockback: Vec3, source: PlayerPositionState) = PlayerPositionPacketObservation(
    ...
    fun remotePositionSync(packet: ClientboundEntityPositionSyncPacket) = positionChange(
    ...
    fun velocity(packet: ClientboundSetEntityMotionPacket, local: Boolean) = PlayerPositionPacketObservation(
