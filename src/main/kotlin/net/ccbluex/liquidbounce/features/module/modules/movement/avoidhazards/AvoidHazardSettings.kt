...
internal enum class Avoid(
    override val tag: String,
    val fullCube: Boolean = true,
    val test: (block: Block, fluidState: FluidState, pos: BlockPos) -> Boolean,
) : Tagged {
    CACTI("Cacti", test = { block, _, _ -> block is CactusBlock }),
    BERRY_BUSH("BerryBush", test = { block, _, _ -> block is SweetBerryBushBlock }),
    FIRE("Fire", test = { block, _, _ -> block is FireBlock }),
    COBWEB("Cobwebs", test = { block, _, _ -> block is WebBlock }),
    LADDERS("Ladders", test = { _, _, pos -> isLadderClimbStateAt(pos) }),
    ...
    WITHER_ROSE("WitherRose", test = { block, _, _ -> block is WitherRoseBlock }),
    POWDER_SNOW("PowderSnow", test = { block, _, _ -> block is PowderSnowBlock }),
}

internal fun isLadderClimbStateAt(pos: BlockPos): Boolean {
