    ...
    private val renderHandler = handler<WorldRenderEvent> { event ->
        ...
        event.renderEnvironment {
            for (entity in world.players()) {
                ...
                val vanillaBodyRot = rotLerp(event.partialTicks, entity.yBodyRotO, entity.yBodyRot)
                val shouldUseModelRotation = shouldUseModelWingsRotation(
                    isLocalPlayer = isMe,
                    playerModelRunning = ModulePlayerModel.running,
                    displayMode = ModulePlayerModel.displayMode,
                    rotationStateEnabled = ModulePlayerModel.isStateEnabled(ModulePlayerModel.State.ROTATION),
                    bodyPartAllowed = ModulePlayerModel.isPartAllowed(ModulePlayerModel.BodyPart.BODY),
                )
                val bodyRot = resolveWingsBodyRotation(
                    vanillaBodyRotation = vanillaBodyRot,
                    modelBodyRotation = ModulePlayerModel.interpolatedModelRotation(event.partialTicks)?.yaw,
                    useModelRotation = shouldUseModelRotation,
                )
