...
/**
 * A readonly [Collection] containing all [LivingEntity] instances accepted by the installed visibility policy.
 *
...
object RenderedEntities : Collection<LivingEntity> by entities, EventListener {
    ...
    private val visibilityPolicy = RenderedEntityVisibilityPolicy<LivingEntity>()
    ...
    fun installEntityFilter(filter: (LivingEntity) -> Boolean) {
        entityFilter = filter
    }

    internal fun installVisibilityPolicy(
        shouldRenderEntity: (LivingEntity) -> Boolean,
        shouldRefreshOnPerspective: () -> Boolean,
    ) {
        visibilityPolicy.install(shouldRenderEntity, shouldRefreshOnPerspective)
    }
    ...
    private fun refresh() {
        ...
        for (entity in mc.level?.entitiesForRendering() ?: return) {
            if (entity is LivingEntity && visibilityPolicy.shouldRender(entity)) {
    ...
    private val perspectiveChangeHandler = handler<PerspectiveEvent> {
        if (visibilityPolicy.shouldRefreshOnPerspective()) {
            refresh()
        }
    }
