...
import net.ccbluex.liquidbounce.features.rotation.processors.anglesmooth.FactorAngleSmooth
...
class InterpolationAngleSmooth(
    ...
    override fun calculateFactors(
        ...
        DebugParameterSink.publish(this, "Yaw Diff") { yawDiff }
        DebugParameterSink.publish(this, "Pitch Diff") { pitchDiff }
        ...
        DebugParameterSink.publish(this, "Direction Change") { directionChange }
        ...
        DebugParameterSink.publish(this, "Horizontal Speed") { horizontalSpeed }
        DebugParameterSink.publish(this, "Vertical Speed") { verticalSpeed }
    ...
    private fun calculateFactor(name: String, rotationDifference: Float, turnSpeed: Float,
        ...
        DebugParameterSink.publish(this, "$name T") { t }

        val bezierSpeed = bezier(0.05f, 1f, 1f - t)
        val sigmoidSpeed = sigmoid(t)

        DebugParameterSink.publish(this, "$name Bezier") { bezierSpeed }
        DebugParameterSink.publish(this, "$name Sigmoid") { sigmoidSpeed }

        return if (t > midpoint) {
            DebugParameterSink.publish(this, "$name R") { "Bezier" }
            bezierSpeed * turnSpeed
        } else {
            DebugParameterSink.publish(this, "$name R") { "Sigmoid" }
