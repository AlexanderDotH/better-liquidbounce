...
class ViewedInventoryScreen(private val player: () -> Player?) : Screen(PlainText.EMPTY) {
    ...
    override fun extractRenderState(context: GuiGraphicsExtractor, mouseX: Int, mouseY: Int, delta: Float) {
        ...
        val hoveredSlot = drawSlotsAndFindHoveredSlot(context, handler, mouseX, mouseY)
    ...
    private fun drawSlotsAndFindHoveredSlot(
        context: GuiGraphicsExtractor,
        handler: InventoryMenu,
        mouseX: Int,
        mouseY: Int,
    ): Slot? {
        var hoveredSlot: Slot? = null
        val relativeMouseX = mouseX.toDouble() - x
        val relativeMouseY = mouseY.toDouble() - y

        for (slot in handler.slots) {
            if (!slot.isActive) continue
            drawSlot(context, slot)
            if (!isPointOverViewedInventorySlot(slot.x, slot.y, relativeMouseX, relativeMouseY)) continue
