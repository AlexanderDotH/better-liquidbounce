...
object PlayerModelParticleHook {
    ...
    fun shouldRedirectEntityParticles(entity: Entity): Boolean {
        if (!AmnesiaPlayerModelBridge.isRunning()) {
            return false
        }

        val target = AmnesiaPlayerModelBridge.findTarget() ?: return false
    ...
    fun getParticleRedirectOffset(x: Double, y: Double, z: Double): Vec3? {
        ...
        val target = AmnesiaPlayerModelBridge.findTarget() ?: return null
    ...
    private fun getActiveOffset(): Vec3? {
        ...
        val target = AmnesiaPlayerModelBridge.findTarget() ?: return null
        val visualPos = AmnesiaPlayerModelBridge.visualTransform(target)?.position ?: return null
