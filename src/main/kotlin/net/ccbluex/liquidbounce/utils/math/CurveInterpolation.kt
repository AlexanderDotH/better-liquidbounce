...
internal object CurveDataNormalizer {

    /**
     * Normalize data by sorting points by X and removing duplicates.
     */
    fun sortAndDeduplicateByX(data: List<Vector2fc>): List<Vector2fc> {
        if (data.size <= 1) {
            return data
        }
    ...
    fun normalizeTension(tension: Float): Float {
        return if (tension.isInfinite() || tension.isNaN()) 0f else tension.coerceIn(0f, 1f)
    }
}

internal object CurveInterpolation {

    fun transformNormalized(
        data: List<Vector2fc>,
        xPos: Float,
        tension: Float,
        onOutOfBounds: CurveUtil.OnOutOfBounds,
    ): Float {
        if (data.size == 1) {
            return data[0].y()
        }
    ...
    fun extrapolateLinear(data: List<Vector2fc>, xPos: Float, isLeftSide: Boolean): Float {
