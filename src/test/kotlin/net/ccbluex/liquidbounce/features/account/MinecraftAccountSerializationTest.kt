...
class MinecraftAccountSerializationTest {
    ...
    @Test
    fun `malformed account errors never include credential-bearing JSON`() {
        val secret = "super-secret-refresh-token"
        val error = assertFailsWith<IllegalArgumentException> {
            MinecraftAccount.fromJson(parse("""{"name":"Player","refreshToken":"$secret"}"""))
        }
    ...
    @Test
    fun `round-trips a microsoft account saved before the MinecraftAuth migration without logging in`() {
        val json = parse(
            """{"name":"Player","refreshToken":"token","authMethod":"MINECRAFT_PC","type":"MicrosoftAccount"}"""
        )

        val account = MinecraftAccount.fromJson(json)
        val persisted = account.toJson()

        assertTrue(account is MicrosoftAccount)
        assertEquals("Player", account.username)
        assertEquals("token", persisted["refreshToken"].asString)
        assertEquals("MINECRAFT_PC", persisted["authMethod"].asString)
        assertTrue(!persisted.has("authManager"))
    }

    @Test
    fun `round-trips a new MinecraftAuth account without refreshing or exposing credentials`() {
        val json = parse(
            """
            {
              "name": "Player",
              "uuid": "069a79f4-44e9-4726-a5be-fca90e38aaf5",
              "authManager": {"_saveVersion": 1, "opaqueState": "preserved"},
