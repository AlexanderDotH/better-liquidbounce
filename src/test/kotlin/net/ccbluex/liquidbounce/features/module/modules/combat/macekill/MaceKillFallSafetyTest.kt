...
class MaceKillFallSafetyTest {
    ...
    fun `cancelled movement does not advance delivery confirmed lifecycle`() {
        ...
        val plan = MaceKillServerFallSafetyPlan.createForMovements(
    ...
    private fun completedAirborneLifecycle(movement: Vec3): MaceKillFallSafetyLifecycle {
        val ready = MaceKillServerFallSafetyPlan.createForMovements(
            movements = listOf(movement),
            outboundStepCount = 0,
            initialFallDistance = 0.0,
            safeFallDistance = 3.0,
        ...
        return MaceKillFallSafetyLifecycle().apply {
            begin(ready.plan)
